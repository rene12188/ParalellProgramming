using Project.ConsoleApplication.Models;

namespace Project.DataGenerator;

public class ImportCommunityFactory
{
    readonly List<ImportCommunity> _communities = new List<ImportCommunity>();
    
    public void AddCommunity(string name)
    {
        if (_communities.Any(c => c.Name == name))
        {
            throw new ArgumentException("Community already exists");
        }
