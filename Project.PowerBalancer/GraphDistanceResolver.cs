...
public class GraphDistanceResolver
{
    ...
    public void FloydWarshall()
    {
        ...
        foreach (string j in _adjacencyMatrix.Keys)
            if (_adjacencyMatrix[i].ContainsKey(k) && _adjacencyMatrix[k].ContainsKey(j) && (!_adjacencyMatrix[i].ContainsKey(j) || _adjacencyMatrix[i][j] > _adjacencyMatrix[i][k] + _adjacencyMatrix[k][j]))
